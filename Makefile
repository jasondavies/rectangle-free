...
PARTITION_POLY_7_CACHE_CFLAGS ?= -DRAW_CACHE_BITS=14 -DRAW_CACHE_PROBE=16 -DCACHE_PROBE=12
...
partition_poly_7: $(PARTITION_SHARED_SRCS) $(NAUTY_BUILD_DIR)/nautyTL1.a
	$(CC) $(PARTITION_CFLAGS) $(PARTITION_POLY_DEFAULT_ADAPTIVE_CFLAGS) $(PARTITION_POLY_7_NAUTY_CFLAGS) $(PARTITION_POLY_7_CACHE_CFLAGS) -DMAX_ROWS=7 -DMAX_COLS=7 -DDEFAULT_ROWS=7 -DDEFAULT_COLS=7 -DCACHE_BITS=18 -o $@ $(PARTITION_SHARED_SRCS) $(PARTITION_POLY_7_LDFLAGS)

partition_poly_7_profile: $(PARTITION_SHARED_SRCS) $(NAUTY_BUILD_DIR)/nautyTL1.a
	$(CC) $(PARTITION_CFLAGS) $(PARTITION_PROFILE_CFLAGS) $(PARTITION_POLY_DEFAULT_ADAPTIVE_CFLAGS) $(PARTITION_POLY_7_NAUTY_CFLAGS) $(PARTITION_POLY_7_CACHE_CFLAGS) -DMAX_ROWS=7 -DMAX_COLS=7 -DDEFAULT_ROWS=7 -DDEFAULT_COLS=7 -DCACHE_BITS=18 -o $@ $(PARTITION_SHARED_SRCS) $(PARTITION_POLY_7_LDFLAGS)
...
connected_canon_lookup_gen: connected_canon_lookup_gen.c src/runtime.c src/partitions.c src/poly.c src/graph.c src/cache.c src/solver.c $(NAUTY_BUILD_DIR)/nautyTL1.a
	$(CC) $(PARTITION_CFLAGS) $(PARTITION_POLY_7_NAUTY_CFLAGS) $(PARTITION_POLY_7_CACHE_CFLAGS) -DMAX_ROWS=7 -DMAX_COLS=7 -DDEFAULT_ROWS=7 -DDEFAULT_COLS=7 -DCACHE_BITS=18 -o $@ connected_canon_lookup_gen.c src/runtime.c src/partitions.c src/poly.c src/graph.c src/cache.c src/solver.c $(PARTITION_POLY_7_LDFLAGS)
