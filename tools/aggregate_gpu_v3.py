...
if __package__:
    from .gpu_result_v3 import (ValidationError, SHA256, PROVENANCE_FIELDS,
                               parse_nonnegative, read_result as read_v3_result)
else:
    from gpu_result_v3 import (ValidationError, SHA256, PROVENANCE_FIELDS,
                              parse_nonnegative, read_result as read_v3_result)
...
GEOMETRIES = {
    ...
    "6x12": Geometry(
        "6x12",
        "T_4(6,12)",
        "RECT6X12_PREFIX_RESULT",
        b"R6W1202\0",
        12,
        20_230_535_486,
        2_582_439_511_573_240_996_450,
        1 << 72,
    ),
    "7x9": Geometry(
        "7x9",
...
def read_result(path: Path, item: WorkItem, geometry: Geometry) -> dict[str, Any]:
    identity = dict(id=item.id, path=item.path, start=str(item.start),
                    end=str(item.end), filter_mod=str(item.filter_mod),
                    filter_id=str(item.filter_id))
    return read_v3_result(path, magic=geometry.result_magic, identity=identity,
                          geometry=geometry.name,
                          transpose_quotient=geometry.transpose_quotient)
