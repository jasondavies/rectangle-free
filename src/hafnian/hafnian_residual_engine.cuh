// Shared persistent exact residual-hafnian campaign engine.
#pragma once
...
template<class Campaign>
struct Engine {

using Clock=std::chrono::steady_clock;
using Catalog=typename Campaign::Catalog;
using Query=typename Campaign::Query;
#if defined(HAFNIAN_RUNTIME_MONTGOMERY_CONTROL)
static constexpr const char* ALGORITHM="glynn-trace-hessenberg-residual-runtime-montgomery-control-v1";
static constexpr const char* FORMAT=Campaign::CONTROL_FORMAT;
#else
static constexpr const char* ALGORITHM="glynn-gray-resolvent-fixed-field-cuda-v7";
static constexpr const char* FORMAT=Campaign::FORMAT;
#endif
    ...
    if constexpr(N>=48&&N<=58) {
        ...
        if(options.threads&&options.threads!=hafnian_gray::THREADS)
            throw std::runtime_error(
                "Gray-enabled orders use a fixed 64-thread kernel");
...
std::string dispatch_mod(const Query& query,const Catalog& catalog,const Task& task,
    ...
    switch(query.vertices) {
        ...
        case 50:if constexpr(Campaign::WIDTH==28) return run_query<50,Mod>(query,catalog,task,options,workspace,binary_digest);
            else throw std::runtime_error("order outside campaign");
        case 52:if constexpr(Campaign::WIDTH==28) return run_query<52,Mod>(query,catalog,task,options,workspace,binary_digest);
            else throw std::runtime_error("order outside campaign");
        ...
        case 60:if constexpr(Campaign::WIDTH==28) return run_query<60,Mod>(query,catalog,task,options,workspace,binary_digest);
            else throw std::runtime_error("order outside campaign");
        case 64:if constexpr(Campaign::WIDTH==28) return run_query<64,Mod>(query,catalog,task,options,workspace,binary_digest);
            else throw std::runtime_error("order outside campaign");
...
int main(int argc,char** argv) try {
    ...
    auto catalog=Campaign::build_catalog();
    std::printf(
        "HAFNIAN_6X%u_CATALOG queries=%zu digest=%s matrix_padding=%u exact=OK\n",
        Campaign::WIDTH,catalog.queries.size(),catalog.digest.c_str(),1U);
    if(options.list) {
        for(const Query& query:catalog.queries)
            std::printf(
                "HAFNIAN_6X%u_QUERY id=%u occupied=%" PRIu64
                " defects=%u excess=%u unmatched=%u coefficient=%" PRIu64
                " vertices=%u terms=%" PRIu64 " matching_bound_power=%u digest=%s\n",
                Campaign::WIDTH,query.id,query.occupied,query.defect_count,query.excess,
