...
// Exact four-term Gray chains evaluated from one tridiagonalization.  The
// three later rank-two changes are contracted through a truncated resolvent
// determinant, avoiding three further generalized-Lanczos rebuilds.
...
__global__ __launch_bounds__(THREADS,MIN_BLOCKS_PER_SM) void terms_kernel(
    ...
    static_assert(N==48,"the measured resolvent backend is specialized for order 48");
    ...
    uint32_t* future=scratch+2*slots*N*N+slot*M*N;
    uint32_t* state=scratch+2*slots*N*N+slots*M*N+slot*8*N;
    uint32_t* metric=state;
    uint32_t* inverse_metric=metric+N;
    uint32_t* diagonal=inverse_metric+N;
    uint32_t* beta=diagonal+N;
    uint32_t* deltas=beta+N;
    ...
    for(uint64_t chain_begin=begin+uint64_t(slot)*CHAIN;
        ...
        for(unsigned attempt=0;attempt<4&&!ok;++attempt)
            ok=hafnian_gray::generalized_lanczos(
                ...
                mod,basis,metric,inverse_metric,diagonal,beta,
        ...
        uint32_t contribution=term_from_characteristic<N>(
            first,signs0,mod,inverse_small,second);
        ...
        for(unsigned pivot=0;pivot<M;++pivot) {
            ...
            if(pivot&1) {
                ...
                contribution=term_from_characteristic<N>(
                    third,signs,mod,inverse_small,second);
                if(lane==0)local_sum=hafnian_add_mod(local_sum,contribution,mod.p);
                __syncwarp();
            }
        }
...
constexpr size_t scratch_words(size_t slots) {
    return size_t(2)*slots*N*N+slots*UPDATE_COLUMNS*N+slots*8*N;
}
