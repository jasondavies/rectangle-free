...
#ifndef HAFNIAN_GRAY_WARP_CHAIN
#define HAFNIAN_GRAY_WARP_CHAIN 1
#endif

#ifndef HAFNIAN_GRAY_MIN_BLOCKS
#define HAFNIAN_GRAY_MIN_BLOCKS 16
#endif
...
template<class Mod>
__device__ uint32_t warp_dot_metric(const uint32_t* left,const uint32_t* right,
...
template<class Mod>
__device__ void warp_dot_metric_pair(
    const uint32_t* left0,const uint32_t* left1,const uint32_t* right,
    const uint32_t* metric,unsigned rank,Mod mod,uint32_t& result0,uint32_t& result1) {
    const unsigned lane=threadIdx.x&31;
    uint32_t value0=0,value1=0;
...
template<class Mod>
__device__ void warp_apply_dense(const uint32_t* matrix,const uint32_t* input,
    uint32_t* output,unsigned rank,Mod mod) {
    const unsigned lane=threadIdx.x&31;
    for(unsigned row=lane;row<rank;row+=32) {
        uint32_t sum=0;
...
template<class Mod>
__device__ void warp_apply_tridiagonal_update(
    const uint32_t* diagonal,const uint32_t* beta,const uint32_t* metric,
    const uint32_t* z0,const uint32_t* z1,uint32_t delta,
    const uint32_t* input,uint32_t* output,unsigned rank,Mod mod) {
    uint32_t projection0=0,projection1=0;
    warp_dot_metric_pair(z0,z1,input,metric,rank,mod,projection0,projection1);
    const unsigned lane=threadIdx.x&31;
    for(unsigned row=lane;row<rank;row+=32) {
        uint32_t value=hafnian_mul(diagonal[row],input[row],mod);
        if(row)value=hafnian_add_mod(value,input[row-1],mod.p);
...
template<class Mod>
__device__ bool warp_generalized_lanczos(
    const uint32_t* dense,const uint32_t* old_diagonal,const uint32_t* old_beta,
    const uint32_t* low0,const uint32_t* low1,uint32_t delta,
    const uint32_t* old_metric,unsigned rank,uint64_t seed,Mod mod,
    uint32_t* basis,uint32_t* new_metric,uint32_t* inverse_new_metric,
    ...
    __syncwarp();
    uint32_t norm=warp_dot_metric(current,current,old_metric,rank,mod);
    if(!norm)return false;
    ...
    for(unsigned column=0;column<rank;++column) {
        ...
        if(dense)warp_apply_dense(dense,current,applied,rank,mod);
        else warp_apply_tridiagonal_update(old_diagonal,old_beta,old_metric,
            low0,low1,delta,current,applied,rank,mod);
        const uint32_t numerator=warp_dot_metric(
            current,applied,old_metric,rank,mod);
        if(lane==0) {
            inverse_new_metric[column]=hafnian_power(norm,mod.p-2,mod);
            new_metric[column]=norm;
            new_diagonal[column]=hafnian_mul(
        ...
        __syncwarp();
        if(column+1==rank)break;
        const uint32_t next_norm=warp_dot_metric(next,next,old_metric,rank,mod);
        if(!next_norm)return false;
...
template<class Mod>
__device__ void warp_inverse_basis_apply_pair(
    const uint32_t* basis,const uint32_t* old_metric,const uint32_t* inverse_new_metric,
    const uint32_t* input0,const uint32_t* input1,uint32_t* output0,uint32_t* output1,
    unsigned rank,Mod mod) {
    const unsigned lane=threadIdx.x&31;
...
template<unsigned N,class Mod>
__device__ uint32_t warp_term_from_tridiagonal(
    const uint32_t* diagonal,const uint32_t* beta,uint64_t signs,unsigned rank,
    Mod mod,const uint32_t* inverse_small,uint32_t* poly) {
    constexpr unsigned HALF=N/2,STRIDE=HALF+1;
    const unsigned lane=threadIdx.x&31;
...
template<unsigned N,unsigned CHAIN,class Mod>
__global__ __launch_bounds__(64,HAFNIAN_GRAY_MIN_BLOCKS) void gray_warp_update_kernel(
    const uint32_t* __restrict__ edge_matrices,
    const uint32_t* __restrict__ update_vectors,
    const uint32_t* __restrict__ fixed_metric,unsigned rank,
    uint64_t begin,uint64_t end,Mod mod,const uint32_t* __restrict__ inverse_small,
    ...
    uint32_t* __restrict__ failures) {
    static_assert(CHAIN>=1&&CHAIN<=8);
    constexpr unsigned HALF=N/2,WARPS=2;
    const unsigned lane=threadIdx.x&31,warp=threadIdx.x>>5;
    const size_t slot=size_t(blockIdx.x)*WARPS+warp;
    const size_t slots=size_t(gridDim.x)*WARPS;
    extern __shared__ uint32_t shared[];
    ...
    for(uint64_t chain_begin=begin+uint64_t(slot)*CHAIN;
        ...
        bool ok=false;
        for(unsigned attempt=0;attempt<4&&!ok;++attempt)
            ok=warp_generalized_lanczos(
                dense,nullptr,nullptr,nullptr,nullptr,0,fixed_metric,rank,
                splitmix64(chain_begin)^attempt,mod,basis,metrics,inverse_metrics,
                diagonals,betas,previous,current,next,applied);
        if(!ok) {
            ++local_failures;
            continue;
        }
        for(unsigned future_step=1;future_step<chain_end-chain_begin;++future_step) {
            uint32_t* first=future+(size_t(future_step-1)*2+0)*N;
            uint32_t* second=future+(size_t(future_step-1)*2+1)*N;
            warp_inverse_basis_apply_pair(basis,fixed_metric,inverse_metrics,
                first,second,temporary0,temporary1,rank,mod);
            for(unsigned row=lane;row<rank;row+=32) {
                first[row]=temporary0[row];
                second[row]=temporary1[row];
            }
            __syncwarp();
        }
        uint32_t contribution=warp_term_from_tridiagonal<N>(
            diagonals,betas,signs0,rank,mod,inverse_small,poly);
        ...
        for(uint64_t index=chain_begin+1;index<chain_end;++index) {
            ...
            ok=false;
            for(unsigned attempt=0;attempt<4&&!ok;++attempt)
                ok=warp_generalized_lanczos(nullptr,diagonals+current_buffer*N,
                    betas+current_buffer*N,z0,z1,delta,old_metric,rank,
            ...
            for(unsigned future_step=stage+1;
                ...
                uint32_t* first=future+(size_t(future_step-1)*2+0)*N;
                uint32_t* second=future+(size_t(future_step-1)*2+1)*N;
                warp_inverse_basis_apply_pair(basis,old_metric,
                    inverse_metrics+next_buffer*N,first,second,
                    temporary0,temporary1,rank,mod);
                for(unsigned row=lane;row<rank;row+=32) {
                    first[row]=temporary0[row];
            ...
            current_buffer=next_buffer;
            contribution=warp_term_from_tridiagonal<N>(diagonals+current_buffer*N,
                betas+current_buffer*N,signs,rank,mod,inverse_small,poly);
            if(lane==0)local_sum=hafnian_add_mod(local_sum,contribution,mod.p);
            __syncwarp();
        }
...
void run_probe(const Query& query,const RankFactor& factor,uint64_t terms,
...
#if HAFNIAN_GRAY_WARP_CHAIN
    constexpr size_t dynamic_shared=size_t(2)*4*N*sizeof(uint32_t);
    hafnian_cuda_check(cudaFuncSetAttribute(gray_warp_update_kernel<N,CHAIN,Mod>,
        cudaFuncAttributeMaxDynamicSharedMemorySize,int(dynamic_shared)),
        "set warp-Gray dynamic shared memory");
    int active=0;
    hafnian_cuda_check(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active,
        gray_warp_update_kernel<N,CHAIN,Mod>,THREADS,dynamic_shared),
        "query warp-Gray occupancy");
    const unsigned requested_slots=requested_blocks?requested_blocks:
        unsigned(multiprocessors*active*2*2);
    const unsigned gray_grid=(requested_slots+1)/2;
    const unsigned gray_slots=gray_grid*2;
#else
    ...
    const unsigned gray_grid=requested_blocks?requested_blocks:
        unsigned(multiprocessors*active*2);
    const unsigned gray_slots=gray_grid;
#endif
    if(!active)throw std::runtime_error("Gray kernel has zero occupancy");
    ...
    const unsigned maximum_blocks=std::max(gray_slots,baseline_blocks);
...
#if HAFNIAN_GRAY_WARP_CHAIN
    const size_t scratch_words=size_t(2)*gray_slots*N*N+
        size_t(gray_slots)*2*(CHAIN-1)*N+size_t(gray_slots)*8*N;
#else
    const size_t scratch_words=size_t(2)*gray_slots*N*N+
        (HAFNIAN_GRAY_GLOBAL_FUTURE?
            size_t(gray_slots)*2*(CHAIN-1)*N:0);
#endif
...
#if HAFNIAN_GRAY_WARP_CHAIN
        gray_warp_update_kernel<N,CHAIN,Mod><<<gray_grid,THREADS,dynamic_shared>>>(
...
#else
        gray_update_kernel<N,CHAIN,Mod><<<gray_grid,THREADS,dynamic_shared>>>(
            input.edge_matrices,input.update_vectors,input.metric,factor.rank,
            0,terms,mod,input.inverse_small,device_dense,device_sums,device_failures);
#endif
    },gray_slots,gray_residue,gray_failures);
    ...
    std::printf(
        ...
        " prime=%u warp_chain=%d future_global=%d gray_blocks=%u gray_slots=%u "
        "active_blocks_per_sm=%d shared_bytes=%zu "
        ...
        N,factor.rank,CHAIN,terms,mod.p,HAFNIAN_GRAY_WARP_CHAIN,
        HAFNIAN_GRAY_GLOBAL_FUTURE,gray_grid,gray_slots,active,dynamic_shared,
